...
class CsvParserTest {
    ...
    @Test
    fun delimiterInsideQuotesIsNotCounted() {
    ...
    @Test
    fun headerOffSynthesisesColumnNamesAndKeepsFirstRow() {
    ...
    @Test
    fun deduplicatesRepeatedHeaders() {
        val p = CsvParser.parse("word,word,word\na,b,c", hasHeader = true)
        assertEquals(listOf("word", "word_2", "word_3"), p.headers)
    }

    @Test
    fun ragdedRowsArePaddedToHeaderWidth() {
        val p = CsvParser.parse("a,b,c\n1,2\n3,4,5", hasHeader = true)
        assertEquals(listOf(listOf("1", "2", ""), listOf("3", "4", "5")), p.rows)
    }

    @Test
    fun bodyRowWiderThanHeaderPadsHeaderWidth() {
        val p = CsvParser.parse("a,b\n1,2,3", hasHeader = true)
        assertEquals(listOf("a", "b", ""), p.headers)
        assertEquals(listOf(listOf("1", "2", "3")), p.rows)
    }

    @Test
    fun emptyInputYieldsEmpty() {
        val p = CsvParser.parse("", hasHeader = true)
        assertEquals(emptyList<String>(), p.headers)
        assertEquals(emptyList<List<String>>(), p.rows)
    }

    @Test
    fun whitespaceOnlyInputYieldsEmpty() {
        val p = CsvParser.parse("   \n  ", hasHeader = false)
        assertEquals(emptyList<String>(), p.headers)
        assertEquals(emptyList<List<String>>(), p.rows)
    }
}
